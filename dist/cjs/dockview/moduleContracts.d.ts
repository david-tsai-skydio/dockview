...
import { DroptargetOverlayModel, Position, PositionResolver, PositionResolverArgs, PositionResolverResult } from '../dnd/droptarget';
...
import { DockviewComponentOptions, FloatingGroupDragContext, SmartGuidesOptions } from './options';
import { DockviewActivePanelChangeEvent, DockviewLayoutMutationEvent, DockviewLayoutMutationKind, DockviewOrigin, DockviewPanelPinnedChangeEvent, GroupNavigationDirection, SerializedDockview } from './dockviewComponent';
...
export interface ISmartGuidesHost {
     ...
     */
    mergeFloatInto(dragged: DockviewGroupPanel, target: DockviewGroupPanel, position: SmartGuidesSnapPosition): void;
}
/** Where a snapped-together float docks relative to its target. */
export type SmartGuidesSnapPosition = 'left' | 'right' | 'top' | 'bottom' | 'center';
...
export interface IPinnedTabsService extends IDisposable {
    ...
    resolveDropIndex(group: DockviewGroupPanel, panelId: string, index: number): number;
}
/**
